...
//template <typename T, template <typename, typename> class Container> //不能这么写，下面container<T>会少一个参数，但是你上面必须写俩必须写分配器。
//--------------------------------------------
// template <typename T>
...
int main(){
    ...
    Stack<int,vector> ss; //一个stack，实现基于vector类，储存int类型数据
