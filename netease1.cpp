...
int main() {
    ...
    for (auto& i : node) {
        if (cur_pivot == i.first) {
            if (color_map[i.first] == color_map[i.second]) {
                candidate[i.first].emplace_back(i.second);
            }
        }
        else {
            cur_pivot++;
    ...
    for (auto& i : candidate) {
        sizes.emplace_back(i.first, i.second.size());
    }
    
    sort(sizes.begin(), sizes.end(), mysort());
    if (candidate.empty()) {
        cout << "0" << endl;
        return 0;
    }
    auto f = candidate[sizes[0].first];
    cout << f.size()+1 << endl;
    cout << sizes[0].first<< endl;
    for (auto& i : f) {
        cout << i << endl;
    }
    return 0;
