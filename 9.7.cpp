...
void accumulate2(std::vector<int>::iterator first,
                std::vector<int>::iterator last,
                std::unique_ptr<int> my_ptr, std::promise<int> accumulate_promise)
{
    int sum = std::accumulate(first, last, 0);
...
int main()
{
    ...
    std::vector<int> numbers = { 1, 2, 3, 4, 5, 6 };
    std::unique_ptr<int> ptr = std::make_unique<int>(44);
    std::promise<int> accumulate_promise;
    std::future<int> accumulate_future(accumulate_promise.get_future());
    std::thread work_thread(accumulate2, numbers.begin(), numbers.end(), move(ptr),
...
//     std::vector<int> numbers = { 1, 2, 3, 4, 5, 6 };
//     std::future<int> accumulate_future = std::async(std::launch::async, accumulate3, numbers.begin(), numbers.end());
